...
export function takeOperators ( ) {
	...
	return [((operators_array.length > 0)?true:(document.getElementById('operators_warn').textContent = 'Выберете хотя бы один оператор!', document.getElementById('operators_warn').style.color = 'red', false)), operators_array.split('')];
}

export function clearWarns ( ) {
	document.getElementById('operators_warn').textContent = '';
